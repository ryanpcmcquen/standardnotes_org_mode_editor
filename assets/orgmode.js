...
CodeMirror.defineSimpleMode("orgmode", {
    start: [
        {
            regex: /^(^\*{1,6}\s)(TODO|DOING|WAITING|NEXT){0,1}(CANCELLED|CANCEL|DEFERRED|DONE|REJECTED|STOP|STOPPED){0,1}(.*)$/,
            token: [
...
CodeMirror.afterInit = function(editor) {
        ...
        "Shift-Tab": function(cm) {
            if (state.stab === "SHOW_ALL") {
                ...
                cm.operation(function() {
                    for (
                        var i = cm.firstLine(), e = cm.lastLine();
                        i <= e;
                        i++
                    ) {
                ...
                cm.operation(function() {
                    for (
                        ...
                        if (
                            /header/.test(
                                cm.getTokenTypeAt(CodeMirror.Pos(i, 0))
                            ) === true
                        ) {
    ...
    editor.on("touchstart", function(cm, e) {
        setTimeout(() => {
            isFold(cm, cm.getCursor())
                ? unfold(cm, cm.getCursor())
    ...
    editor.operation(function() {
        for (var i = 0; i < editor.lineCount(); i++) {
            if (
                /header/.test(editor.getTokenTypeAt(CodeMirror.Pos(i, 0))) ===
                false
            ) {
